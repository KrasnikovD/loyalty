...
class ImportUsers extends Command
{
    ...
    {
        $ids = [66,1100,1532,1818,1964,3071,3801,3941,4356];
        foreach (Users::whereIn('id', $ids)->get() as $item1) {
            $userId = $item1->id;
            print $userId."\n";
            foreach (Cards::where('user_id', $userId)->get() as $item) {
                foreach (Bills::where('card_id', $item->id)->get() as $value) {
                    BonusHistory::where('bill_id', $value->id)->delete();
                }
                Bills::where('card_id', $item->id)->delete();
                CardHistory::where('card_id', $item->id)->delete();
            }
            Cards::where('user_id', $userId)->forceDelete();
        }
        Users::whereIn('id', $ids)->delete();

        $pathToGuest = $this->argument('f');
        if (!file_exists($pathToGuest))
